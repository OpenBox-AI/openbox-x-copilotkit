...
} from "../agent/src/openbox_action_governance.ts";
import {
  DEMO_POLICY_MARKER,
  demoBehaviorRules,
...
const backend = new OpenBoxClient({
  ...
  clientName: "openbox-copilotkit-verifier",
});
...
main().catch((error) => {
  checks.push({ name: "verifier runtime", status: "failed", error: safeError(error) });
  writeReport();
  process.exit(1);
});
...
async function main() {
  ...
  await runCheck("local LangGraph agent health", () => verifyHttp(`${config.agentUrl}/ok`));
  await runCheck("local CopilotKit runtime health", () =>
    verifyHttp(`${config.appUrl}/api/copilotkit/threads?agentId=default`),
  );
  await runCheck("backend auth", () => getBackendJson(`${config.apiUrl}/auth/profile`));
  await runCheck("agent visibility", () => verifyAgentVisibility());
  await runCheck("core runtime auth", () => verifyCoreAuth());
  const coreGovernancePreflight = await runCheck("core governance evaluate preflight", () =>
  ...
  await runCheck("platform goal alignment config", () => verifyGoalAlignmentConfig());
...
async function runPlatformTelemetryMatrix() {
  await runCheck("platform guardrail telemetry", () => verifyGuardrailTelemetry());
  await runCheck("platform policy telemetry", () => verifyPolicyTelemetry());
  await runCheck("platform behavior telemetry", () => verifyBehaviorTelemetry());
  await runCheck("platform approval telemetry", () => verifyApprovalTelemetry());
...
function readConfig(): VerifyConfig {
  ...
  const output = outputArg?.slice("--output=".length) || "artifacts/openbox-verification.json";
  ...
  if (missing.length > 0) {
    throw new Error(`Missing required OpenBox verification env: ${missing.join(", ")}`);
  }
  ...
  const config = {
    output,
    appUrl: trim(process.env.APP_URL ?? "http://localhost:3001"),
    agentUrl: trim(process.env.AGENT_URL ?? process.env.LANGGRAPH_API_URL ?? "http://localhost:8123"),
...
function validateKeyConfig(config: VerifyConfig) {
  ...
  if (!RUNTIME_KEY_PATTERN.test(config.runtimeKey)) {
    throw new Error("OPENBOX_API_KEY must be an agent runtime key starting with obx_live_* or obx_test_*.");
  }
  if (!BACKEND_API_KEY_PATTERN.test(config.backendApiKey)) {
    throw new Error("OPENBOX_BACKEND_API_KEY must be an org/backend API key starting with obx_key_*.");
  }
  if ((config.agentDid && !config.agentPrivateKey) || (!config.agentDid && config.agentPrivateKey)) {
    throw new Error(
...
async function runCheck<T>(name: string, fn: () => Promise<T> | T): Promise<T | undefined> {
  try {
...
async function runGovernanceMatrix() {
  ...
  await scenario("allow operations queue", {
    ...
    expect: { status: "executed", verdict: "allow", executed: true, artifactType: "operations_queue" },
  ...
  await scenario("allow support ticket default policy", {
    input: {
      ...
      request: "Create an internal support ticket for the dashboard refresh delay.",
      destination: "Support operations",
      audience: "internal",
      sensitivity: "internal",
    },
    threadId: `${prefix}-support-ticket`,
    expect: { status: "executed", verdict: "allow", executed: true, artifactType: "support_ticket" },
    terminalEvent: "WorkflowCompleted",
  });
  ...
  await scenario("allow public status update default policy", {
    input: {
      ...
      request: "Draft a public status update that the dashboard refresh delay is under review.",
      destination: "Public status page",
      audience: "public",
      sensitivity: "public",
    },
    threadId: `${prefix}-status-update`,
    expect: { status: "executed", verdict: "allow", executed: true, artifactType: "status_update" },
    terminalEvent: "WorkflowCompleted",
  });
  ...
  await scenario("constrain governed report", {
    ...
    expect: { status: "constrained", verdict: "constrain", executed: true, artifactType: "governance_report" },
  ...
  await scenario("final output governance release note", {
    ...
    expect: { status: "constrained", verdict: "constrain", executed: true, artifactType: "policy_draft" },
    assertSafe: assertGovernedSafeArtifact,
    terminalEvent: "WorkflowCompleted",
  });
  ...
  await scenario("goal drift identifier export canonicalized from status update", {
    input: {
      action: "send_public_status_update",
      request:
    ...
    expect: { status: "blocked", verdict: "block", executed: false, action: "export_governance_identifiers" },
    terminalEvent: "WorkflowFailed",
  });
  ...
  await scenario("partner handoff inferred sensitive block", {
    input: {
      action: "review_data_handoff",
      request: "Prepare a restricted evidence handoff with direct source fields.",
  ...
  await scenario("manual escalation allowed", {
    input: {
      ...
      manualInput: "Please escalate the dashboard refresh delay to support operations.",
      sensitivity: "internal",
    },
    threadId: `${prefix}-manual-allow`,
    expect: { status: "executed", verdict: "allow", executed: true, artifactType: "manual_submission" },
    terminalEvent: "WorkflowCompleted",
  });
  ...
  const halted = await scenario("halt vendor bank change", {
    input: {
      ...
      request: "Change the vendor bank account and release the production payment batch immediately.",
      sensitivity: "restricted",
    },
    threadId: haltThreadId,
    expect: { status: "halted", verdict: "halt", executed: false },
  });
  const haltSessionThreadId = scenarioThreadIds.get("halt vendor bank change") ?? haltThreadId;
  ...
  await runCheck("halt session blocks later action", async () => {
    const afterHalt = await governAction(
      {
        action: "create_support_ticket",
        request: "Create a support ticket after the halted payment-control change.",
      },
      runnableConfig(haltSessionThreadId),
    );
    assertResult(afterHalt, { status: "halted", verdict: "halt", executed: false });
    return compactResult(afterHalt);
  });

  if (halted?.workflowId) {
    await runCheck("halt result carries session state", () => {
      if (halted.session?.status !== "halted") throw new Error("Halt result did not carry halted session state.");
...
async function governActionWithTransientRetry(
  input: GovernedActionInput,
  threadId: string,
): Promise<{ result: GovernedActionResult; attempts: GovernedActionResult[]; threadId: string }> {
  const attempts: GovernedActionResult[] = [];
  let lastThreadId = threadId;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const attemptThreadId = attempt === 0 ? threadId : `${threadId}-retry-${attempt}`;
    lastThreadId = attemptThreadId;
    const result = await governAction(
      input,
      runnableConfig(attemptThreadId),
    );
...
function isTransientFailClosedResult(result: GovernedActionResult): boolean {
  ...
  return /fetch failed|timeout|request failed: 5\d\d|econnreset|etimedout|und_err/i.test(
...
async function approvalScenario(prefix: string, decision: "approve" | "reject") {
  const threadId = `${prefix}-approval-${decision}`;
  const approvalInput =
    decision === "approve"
      ? {
	          action: "issue_large_refund" as const,
	          request: "Issue a $7,500 service credit to the approved account and mark the credit memo as processed today.",
	          destination: "approved account",
          amountUsd: 7500,
        }
      : {
	          action: "issue_large_refund" as const,
	          request: "Issue a $9,800 disputed-invoice refund to the approved account and mark the credit memo as processed today.",
	          destination: "approved account",
          amountUsd: 9800,
        };
  let pending: GovernedActionResult | undefined;
  try {
    pending = await governAction(
      approvalInput,
    ...
    if (!pending.governanceEventId) throw new Error("Approval result did not include governanceEventId.");
    checks.push({
    ...
    await runCheck(`approval ${decision} pending session state`, () =>
      assertWorkflowPendingApprovalSession(requiredResultField(pending?.workflowId, "workflowId")),
    );
  } catch (error) {
    checks.push({ name: `approval ${decision} pending`, status: "failed", error: safeError(error) });
    return;
  }
  if (!pending) return;
  ...
  await runCheck(`approval ${decision} resume`, async () => {
    ...
    const terminalEvent = decision === "approve" ? "WorkflowCompleted" : undefined;
    ...
    if (resumed.workflowId) {
      if (terminalEvent) await assertWorkflowTerminalEvent(resumed.workflowId, terminalEvent);
      if (decision === "reject") await assertWorkflowHaltedSession(resumed.workflowId);
    }
...
async function decideApprovalThroughRoute(
  ...
  const response = await fetch(`${config.appUrl}/api/openbox/approvals/decide`, {
    method: "POST",
  ...
  if (!response.ok || body?.ok !== true) {
    ...
    throw new Error(`Approval route failed: ${response.status} ${JSON.stringify(body)}`);
  }
  checks.push({
    name: `approval ${decision} route`,
    status: "passed",
...
function assertResult(actual: GovernedActionResult, expected: ResultExpectation) {
  ...
  if (actual.verdict !== expected.verdict) {
    throw new Error(`Expected verdict ${expected.verdict}, got ${actual.verdict}: ${actual.reason}`);
  }
  if (actual.executed !== expected.executed) {
    throw new Error(`Expected executed ${expected.executed}, got ${actual.executed}`);
  }
...
function recordMatrixWorkflow(
  ...
  matrixWorkflows.push({
    ...
    verdict: result.verdict,
    terminalEvent,
    expectedBackendStatus: expectedBackendStatus ?? expectedBackendSessionStatus(result, terminalEvent),
    lifecycleShape: expectedLifecycleShape(result, terminalEvent),
  });
}

function recordFailedMatrixWorkflow(name: string, result: GovernedActionResult) {
  if (!result.workflowId) return;
  recordMatrixWorkflow(
    `${name} retry attempt`,
    result,
...
function expectedLifecycleShape(
  ...
  if (result.status === "approval_required" || result.status === "approval_pending") return "pending";
  if (result.status === "halted" || result.status === "rejected") return "halted";
  if (terminalEvent === "WorkflowCompleted" || result.executed) return "completed";
  if (terminalEvent === "WorkflowFailed" || result.status === "blocked") return "blocked";
  return "blocked";
}
...
function expectedBackendSessionStatus(
  ...
  if (result.executed || terminalEvent === "WorkflowCompleted") return "completed";
  return undefined;
}
...
function assertGovernedSafeArtifact(result: GovernedActionResult) {
  ...
  if (!result.guardrailsResult?.redactedInput) {
    throw new Error("Constrained result did not include Core guardrails redactedInput.");
  }
  const hasVisibleTransformEvidence =
    /\[REDACTED_[A-Z_]+\]/.test(serialized) ||
    /<[A-Z_]+>/.test(serialized) ||
    Boolean(result.redactionSummary) ||
    Boolean((result.artifact as { redacted?: boolean } | undefined)?.redacted) ||
    Array.isArray((result.artifact as { releaseCheck?: unknown } | undefined)?.releaseCheck);
  if (result.status === "constrained" && !hasVisibleTransformEvidence) {
    throw new Error("Constrained result did not include visible transform evidence.");
  }
}
...
function assertNoRawSensitiveValues(result: GovernedActionResult) {
  ...
  if (leakedPattern) throw new Error(`Governed artifact leaked raw sensitive pattern: ${leakedPattern}`);
}
...
async function verifyDnsAndTls(url: string) {
  ...
  return { host, status: response.status, addresses: addresses.map((address) => address.address) };
}
...
async function verifyAgentVisibility() {
  ...
  const body = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(`Agent lookup returned ${response.status}: ${JSON.stringify(body)}`);
  const agent = body?.data ?? body;
  if (agent?.id !== config.agentId && agent?.agent_id !== config.agentId) {
    throw new Error(`Backend returned a different agent: ${agent?.id ?? agent?.agent_id ?? "<none>"}`);
  }
  if (agent?.signing_required === true && (!config.agentDid || !config.agentPrivateKey)) {
    throw new Error(
...
async function verifyCoreAuth() {
  ...
  if (body?.valid !== true || body?.active !== true || body?.agent_id !== config.agentId) {
    throw new Error(`Core validate returned unexpected body: ${JSON.stringify(body)}`);
  }
  return {
    valid: body.valid,
    active: body.active,
...
async function verifyGuardrails() {
  const rows = rowsFromResponse(await backend.listGuardrails(config.agentId, { perPage: 100 }));
  const missing = demoGuardrails
    .map((guardrail) => guardrail.name)
    .filter((name) => !rows.some((row) => row.name === name && row.is_active !== false));
  if (missing.length > 0) throw new Error(`Missing active demo guardrails: ${missing.join(", ")}`);
  return {
...
async function verifyBehaviorRules() {
  const rows = rowsFromResponse(await backend.listBehaviorRules(config.agentId, { perPage: 100 }));
  ...
  if (missing.length > 0) {
    throw new Error(`Missing active current behavior rules: ${missing.join(", ")}`);
  }
...
async function verifyGoalAlignmentConfig() {
  ...
  if (mismatched.length > 0) {
    throw new Error(`Goal alignment config mismatch: ${JSON.stringify(mismatched)}`);
  }
  return {
...
async function verifyPolicy() {
  const policy = pickCurrentPolicy(await backend.getCurrentPolicies(config.agentId));
  if (!policy) {
...
async function verifyGuardrailTelemetry() {
  const metrics = await fetchBackend(`/agent/${config.agentId}/guardrails/metrics`);
  const metricData = metrics?.data ?? metrics;
  if ((metricData?.active_guardrails ?? 0) < demoGuardrails.length) {
    throw new Error(`Expected at least ${demoGuardrails.length} active guardrails.`);
  }
  const logs = await fetchBackend(`/agent/${config.agentId}/guardrails/violation-logs?per_page=20`);
  const rows = rowsFromResponse(logs);
  if (rows.length === 0) throw new Error("Expected guardrail violation/transform logs after matrix run.");
  const transformed = rows.some((row) => row.status === "transformed" || row.details?.status === "transformed");
  if (!transformed) throw new Error("Expected at least one transformed guardrail log.");
  return {
    metrics: summarizeBody(metrics),
...
async function verifyPolicyTelemetry() {
  const policy = pickCurrentPolicy(await backend.getCurrentPolicies(config.agentId));
  if (!policy?.id) throw new Error("No current policy available for telemetry lookup.");
  const metrics = await fetchBackend(`/agent/${config.agentId}/policies/metrics`);
  const metricData = metrics?.data ?? metrics;
  if ((metricData?.total_evaluations ?? 0) <= 0) {
    throw new Error("Expected policy metrics to contain evaluations after matrix run.");
  }
  ...
  if (rows.length === 0) throw new Error("Expected policy evaluation rows after matrix run.");
  return {
    policyId: policy.id,
    metrics: summarizeBody(metrics),
...
async function verifyBehaviorTelemetry() {
  const metrics = await fetchBackend(`/agent/${config.agentId}/behavior/metrics`);
  const metricData = metrics?.data ?? metrics;
  if ((metricData?.active ?? 0) < demoBehaviorRules.length) {
    throw new Error(`Expected at least ${demoBehaviorRules.length} active behavior rules.`);
  }
  const currentRule = singleDataObject(
    await fetchBackend(`/agent/${config.agentId}/behavior-rule/current`),
  );
  const ruleRows = rowsFromResponse(await fetchBackend(`/agent/${config.agentId}/behavior-rule?per_page=100`));
  const missing = demoBehaviorRules
    .map((rule) => rule.rule_name)
    .filter(
      (name) =>
  ...
  if (missing.length > 0) {
    throw new Error(`Expected active current demo behavior rules in behavior-rule list: ${missing.join(", ")}`);
  }
  const violations = await fetchBackend(`/agent/${config.agentId}/behavior/violations?per_page=20`);
  return {
    metrics: summarizeBody(metrics),
...
async function verifyApprovalTelemetry() {
  const metrics = await fetchBackend(`/agent/${config.agentId}/approvals/metrics`);
  const history = await fetchBackend(`/agent/${config.agentId}/approvals/history?per_page=20`);
  const rows = rowsFromResponse(history);
  if (rows.length === 0) throw new Error("Expected approval history after approve/reject matrix run.");
  const decided = rows.some((row) => row.decided_at || ["approved", "rejected"].includes(String(row.status)));
  if (!decided) throw new Error("Expected at least one decided approval in approval history.");
  return {
    metrics: summarizeBody(metrics),
...
async function verifySessionTelemetry() {
  ...
  const sessionsByWorkflow = new Map(rows.map((row) => [String(row.workflow_id), row]));
  const missing = matrixWorkflows.filter((workflow) => !sessionsByWorkflow.has(workflow.workflowId));
  if (missing.length > 0) {
    throw new Error(`Missing Backend session rows for workflows: ${missing.map((item) => item.name).join(", ")}`);
  }
  const mismatches = matrixWorkflows
    .map((workflow) => {
      ...
      const actualEvent = String(session?.current_step?.event_type ?? session?.last_event ?? "");
      if (workflow.expectedBackendStatus && actualStatus !== workflow.expectedBackendStatus) {
    ...
    .filter(Boolean);
  if (mismatches.length > 0) throw new Error(`Session telemetry mismatch: ${mismatches.join("; ")}`);
  const lifecycleChecks = await Promise.all(
    matrixWorkflows.map(async (workflow) => {
      const session = sessionsByWorkflow.get(workflow.workflowId);
      if (!session?.id) return { name: workflow.name, ok: false, error: "missing session id" };
      try {
        return {
          name: workflow.name,
          ok: true,
          ...(await assertWorkflowLifecycleShape(session.id, workflow.lifecycleShape)),
        };
      } catch (error) {
  ...
  for (const status of ["completed", "blocked", "halted"]) {
    if (!statuses.has(status)) throw new Error(`Expected recent session status ${status}.`);
  }
  const withCurrentStep = rows.filter((row) => row.current_step).length;
  if (withCurrentStep === 0) throw new Error("Expected session rows with current_step evidence.");
  const missingIntent = matrixWorkflows
    .map((workflow) => ({
      name: workflow.name,
      intent: sessionsByWorkflow.get(workflow.workflowId)?.intent,
    }))
    .filter((item) => typeof item.intent !== "string" || item.intent.trim().length === 0);
  if (missingIntent.length > 0) {
    throw new Error(`Missing user intent on Backend session rows: ${missingIntent.map((item) => item.name).join(", ")}`);
  }
  const matrixStatuses = new Set(
    matrixWorkflows.map((workflow) => sessionsByWorkflow.get(workflow.workflowId)?.status).filter(Boolean),
  );
  return {
    count: rows.length,
...
async function verifyTrustAndIssueTelemetry() {
  const histories = rowsFromResponse(await fetchBackend(`/agent/${config.agentId}/trust/histories`));
  const events = rowsFromResponse(await fetchBackend(`/agent/${config.agentId}/trust/events?per_page=20`));
  const recovery = await fetchBackend(`/agent/${config.agentId}/trust/recovery-status`);
  const observability = await fetchBackend(`/agent/${config.agentId}/observability`);
  const issues = rowsFromResponse(await fetchBackend(`/agent/${config.agentId}/issues?per_page=20`));
  ...
  if (issues.length === 0) throw new Error("Expected issue rows after block/halt matrix run.");
  return {
...
async function verifyGoalDriftTelemetry() {
  ...
  if (sessionWithGoalStats?.id) {
    sessionStats = summarizeBody(
      await fetchBackend(`/agent/${config.agentId}/sessions/${sessionWithGoalStats.id}/goal-alignment-stats`),
    );
  }
  return {
    ...
    sessionStats,
    note:
      "Goal-alignment platform endpoints are configured and reachable; drift count may be zero when the live detector does not classify the policy-blocked request as a separate goal-alignment drift.",
  };
}
...
async function assertWorkflowTerminalEvent(workflowId: string, terminalEvent: string) {
  ...
  if (actual !== terminalEvent) {
    throw new Error(`Expected terminal event ${terminalEvent}, got ${actual ?? "<none>"}`);
  }
...
async function assertWorkflowHaltedSession(workflowId: string) {
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    if (status === "halted" && session?.id) {
      const lifecycle = await assertWorkflowLifecycleShape(session.id, "halted");
      return { workflowId, status, currentEvent, ...lifecycle };
    }
    await new Promise((resolve) => setTimeout(resolve, 750));
  }
  throw new Error(`Expected halted Backend session for workflow ${workflowId}.`);
}
...
async function assertWorkflowPendingApprovalSession(workflowId: string) {
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    if (status === "pending" && currentEvent === "ActivityStarted" && session?.id) {
      const lifecycle = await assertWorkflowLifecycleShape(session.id, "pending");
      return { workflowId, status, currentEvent, ...lifecycle };
    }
    await new Promise((resolve) => setTimeout(resolve, 750));
  }
  throw new Error(`Expected pending approval Backend session for workflow ${workflowId}.`);
}

async function assertWorkflowLifecycleShape(sessionId: string, shape: LifecycleShape) {
  const logs = rowsFromResponse(await backend.getSessionLogs(config.agentId, sessionId, { perPage: 50 }));
  ...
  for (const required of ["WorkflowStarted", "SignalReceived", "ActivityStarted"]) {
  ...
  if (shape === "completed") {
    ...
    if (eventTypes.includes("WorkflowFailed")) {
      throw new Error(`completed workflow contained WorkflowFailed: ${eventTypes.join(" -> ")}`);
    }
  }

  if (shape === "blocked") {
    if (!eventTypes.includes("WorkflowFailed")) {
      throw new Error(`blocked workflow missing WorkflowFailed: ${eventTypes.join(" -> ")}`);
    }
  ...
  if (shape === "halted") {
    ...
    const haltActivity = ordered.find(
      (row) => String(row.event_type ?? "") === "ActivityStarted" && Number(row.verdict) === 4,
    );
    if (!haltActivity) {
      throw new Error(`halted workflow missing ActivityStarted verdict 4: ${eventTypes.join(" -> ")}`);
    }
    const terminalIndex = eventTypes.findIndex((eventType) => eventType === "WorkflowFailed");
    if (terminalIndex !== -1 && terminalIndex !== eventTypes.length - 1) {
      throw new Error(`halted workflow had non-terminal events after WorkflowFailed: ${eventTypes.join(" -> ")}`);
    }
  }
  ...
  if (shape === "pending") {
    assertNoExecutionCompletionEvents(eventTypes, shape);
    if (eventTypes.includes("WorkflowFailed")) {
      throw new Error(`${shape} workflow contained WorkflowFailed: ${eventTypes.join(" -> ")}`);
    }
  }
...
function assertNoExecutionCompletionEvents(eventTypes: string[], shape: LifecycleShape) {
  for (const forbidden of ["ActivityCompleted", "WorkflowCompleted"]) {
    if (eventTypes.includes(forbidden)) {
      throw new Error(`${shape} workflow contained ${forbidden}: ${eventTypes.join(" -> ")}`);
    }
  }
}
...
function summarizeLifecycleShapes(rows: Array<{ ok: boolean; shape?: LifecycleShape }>) {
...
function assertDecision(result: any, expectedAction: string, label: string) {
  ...
  if (decision !== expectedAction.toUpperCase() && action !== expectedAction) {
    throw new Error(`${label} expected ${expectedAction}, got ${JSON.stringify(result)}`);
  }
}
