...
async function main() {
  ...
  const client = new OpenBoxClient({
    ...
    clientName: "openbox-copilotkit-demo-setup",
  });
...
function readConfig(): DemoConfig {
  ...
  if (!apiKey) throw new Error("OPENBOX_BACKEND_API_KEY is required for setup.");
...
async function verifyDemoSetup(client: OpenBoxClient, agentId: string) {
  ...
  const missingBehaviorRules = demoBehaviorRules
    ...
    .filter((name) => activeByName(behaviorRules, "rule_name", name).length !== 1);
  ...
  if (!policyUsesDemoRules(policy)) {
    throw new Error(`Demo setup verification failed: active policy does not include ${DEMO_POLICY_MARKER}`);
  }
...
async function ensurePolicy(client: OpenBoxClient, config: DemoConfig) {
  const current = pickCurrentPolicy(await client.getCurrentPolicies(config.agentId));
  const nextPolicy = {
...
async function listGuardrails(client: OpenBoxClient, agentId: string): Promise<Array<Record<string, any>>> {
  return rowsFromResponse(await client.listGuardrails(agentId, { perPage: 100 })).filter(
    (row) => typeof row?.name === "string",
  );
}

async function listBehaviorRules(client: OpenBoxClient, agentId: string): Promise<Array<Record<string, any>>> {
  return rowsFromResponse(await client.listBehaviorRules(agentId, { perPage: 100 })).filter(
    (row) => typeof row?.rule_name === "string",
  );
}
