...
export function OpenBoxBusinessActionResult({
  result,
}: {
  result: unknown;
}) {
  ...
  return (
    <section className="openbox-business-result" data-testid="openbox-business-result">
...
function detailRowsFromArtifact(artifact: JsonRecord): Array<[string, string]> {
  return Object.entries(artifact)
    ...
    .map(([key, value]) => [labelForKey(key), textValue(value)] as [string, string])
