...
function timingPayloadFromAgentState(state: unknown): OpenBoxTimingPayload | null {
  const record = recordValue(state);
  return timingPayloadFromTimingValue(recordValue(record[openBoxTimingStateKey]));
}
...
function applyTimingPayload(
  ...
  const now = Date.now();
  const base =
    current && current.action === payload.action && current.request === payload.request
      ? current
      : {
  ...
  const existingIndex = base.steps.findIndex((step) => step.key === payload.event.key);
  const existingStep = existingIndex >= 0 ? base.steps[existingIndex] : undefined;
  const nextStep: OpenBoxLiveTimingStep = {
    ...
    ms:
      payload.event.phase === "finished"
        ? Math.max(0, payload.event.ms ?? now - (existingStep?.startedAtMs ?? now))
        : undefined,
  };
  const steps =
    existingIndex >= 0
      ? base.steps.map((step, index) => (index === existingIndex ? nextStep : step))
      : [...base.steps, nextStep];
  return {
    ...base,
    steps,
